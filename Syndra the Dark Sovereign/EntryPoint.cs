...
namespace Syndra
{
    public class EntryPoint
    {
        ...
        public EntryPoint(Obj_AI_Hero player, Menu menu)
        {
            Player = player;
            Menu = menu;
        }
            ...
            {
                {
                    SpellSlot.Q,
                    new SpellData(
                        LeagueSharp.SpellSlot.Q, TargetSelector.DamageType.Magical, 1, "SyndraQ", 0.6f, 150f, 800f,
                        int.MaxValue, null, @base => @base.GetDarkSphereDamage())
                },
                {
                    SpellSlot.W,
                    new SpellData(
                        LeagueSharp.SpellSlot.W, TargetSelector.DamageType.Magical, 3, "syndrawcast", 0.25f, 210f, 950f,
                ...
                },
                {
                    ...
                    new SpellData(
                        ...
                        () =>
                            (float)
                                (950 *
                                 (ObjectManager.Player.Spellbook.GetSpell(LeagueSharp.SpellSlot.E).Level < 5 ? 0.5 : 1)),
                        @base => @base.GetScatterTheWeakDamage())
                },
                {
                    SpellSlot.R,
                    new SpellData(
                        LeagueSharp.SpellSlot.R, TargetSelector.DamageType.Magical, 2, "SyndraR", 0.5f, 0f, 0f, 0,
                        () => ObjectManager.Player.Spellbook.GetSpell(LeagueSharp.SpellSlot.R).Level < 3 ? 675 : 750,
                        @base => @base.GetUnleashedPowerDamage())
                },
                {
                    SpellSlot.SphereE,
                    new SpellData(
                        LeagueSharp.SpellSlot.E, TargetSelector.DamageType.Magical, 1, "SyndraE", 0.98f, 55f, 1202f,
